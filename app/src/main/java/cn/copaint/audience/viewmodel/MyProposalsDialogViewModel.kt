...
class MyProposalsDialogViewModel: BaseViewModel() {
    val first = 20
    var cursor: Any? = null
    ...
    fun askProposalsInfo(type: Int) {
        job = CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                // 获取到约稿信息
                val response = myApolloClient.apolloClient(ToastUtils.app).query(
                    FindProposalsQuery(
                ...
                Log.i(ToastUtils.app.packageName, response.data.toString())
                ...
                response.data?.proposals?.edges?.forEach {
                    var tempInfo = it?.let { it ->
                        ...
                        val creatorInfoResponse =
                            myApolloClient.apolloClient(ToastUtils.app).query(
